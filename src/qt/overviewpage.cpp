...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BITP)
    {
    }
...
//    ui->label_bitpcoin4->setVisible(showImmature || showWatchOnlyImmature);
...
//        model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
        ...
        connect(ui->blabel_bitpcoin, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

        emit model->makeBalance();
    }

    // update the display unit, to not use the default ("bitpcoin")
    updateDisplayUnit();
}
...
void OverviewPage::updateMasternodeInfo()
{
  ...
  if (masternodeSync.IsBlockchainSynced() && masternodeSync.IsSynced())
  {
   ...
   std::vector<CMasternode> vMasternodes = mnodeman.GetFullMasternodeMap();
    ...
    if (CurrentBlock >= 0) {
        ...
        ui->roi_12->setText(mn1==0 ? " " : QString::number( 3000/roi1,'f',1).append(" days"));
        ui->roi_22->setText(mn2==0 ? " " : QString::number( 11000/roi2,'f',1).append(" days"));
        ui->roi_32->setText(mn3==0 ? " " : QString::number( 55000/roi3,'f',1).append(" days"));
        ui->roi_42->setText(mn4==0 ? " " : QString::number(210000/roi4,'f',1).append(" days"));
    }
    CAmount tNodesSumm = mn1*3000 + mn2*11000 + mn3*55000 + mn4*210000;
  ...
  if (CurrentBlock >= 0) {
      ui->label_lcolat->setText("3000 BITP");
      ui->label_mcolat->setText("11000 BITP");
      ui->label_fcolat->setText("55000 BITP");
      ui->label_pcolat->setText("210000 BITP");
  }
...
void OverviewPage::updateBlockChainInfo()
{
    if (masternodeSync.IsBlockchainSynced())
    {
        ...
        double BlockReward = GetBlockValue(CurrentBlock);
        double BlockRewardbitpcoin =  static_cast<double>(BlockReward/COIN);
        ...
        ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardbitpcoin, 'f', 1).append(" | ") + QString::number(GetSporkValue(SPORK_11_DEV_FEE)).append("%"));

        ui->label_Supply_value->setText(QString::number(chainActive.Tip()->nMoneySupply / COIN).append(" BITP"));
...
void OverviewPage::on_pushButton_Website_clicked() {
    QDesktopServices::openUrl(QUrl("http://www.bitpb.top", QUrl::TolerantMode));
}
void OverviewPage::on_pushButton_Discord_clicked() {
    QDesktopServices::openUrl(QUrl("https://my.sayu.io/", QUrl::TolerantMode));
}
...
void OverviewPage::on_pushButton_Explorer_clicked() {
    QDesktopServices::openUrl(QUrl("https://explorer.bitpb.top", QUrl::TolerantMode));
}
